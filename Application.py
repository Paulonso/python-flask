...
def view_command():
    rows = core.view()
    app.listUsuarios.delete(0, END)
    for r in rows:
        app.listUsuarios.insert(END, r)
...
def getSelectedRow(event):
    global selected
    index = app.listUsuarios.curselection()[0]
    selected = app.listUsuarios.get(index)
...
if __name__ == "__main__":
    app = Gui()
    app.listUsuarios.bind('<<ListboxSelect>>', getSelectedRow)
